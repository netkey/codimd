...
function extractNoteIdFromSocket (socket) {
  ...
  if (get(socket, 'handshake.headers.referer')) {
    ...
    const noteId = config.urlPath ? hostUrl.pathname.slice(config.urlPath.length + 1, hostUrl.pathname.length).split('/')[1] : hostUrl.pathname.split('/')[1]
    return noteId
  }
